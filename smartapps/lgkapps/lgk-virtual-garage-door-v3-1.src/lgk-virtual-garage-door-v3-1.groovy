...
definition(
    name: "LGK Virtual Garage Door v3.1",
    namespace: "lgkapps",
    author: "lgkahn kahn-st@lgk.com",
...
preferences {
    ...
    section("Choose the buzzer that warns when you open/closes? "){
		input "buzzer", "capability.Tone", title: "Pezzo Buzzer?", required: false	
    }
	
    section("Choose the light that warns when you open/closes? "){
		input "flash", "capability.switch", title: "Warning Light?", required: false	
    }
    ...
    if (realgdstate != "open")
      {
        log.debug "opening real gd to correspond with button press"
         ...
         if (buzzer) {buzzer.on()}
         if (flash) {flash.on()}
         opener.on([delay: 5000])
         if (flash) {flash.off([delay: 20000])}
